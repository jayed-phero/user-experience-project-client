...
const MyReview = () => {
    ...
    useEffect(() => {
        fetch(`https://review-assignment-eleven-server.vercel.app/myreviews?email=${user?.email}`, {
            headers: {
    ...
    const handleDelete = (id) => {
        fetch(`https://review-assignment-eleven-server.vercel.app/myreviews/${id}`, {
            method: 'DELETE'
        })
            .then(res => res.json())
            .then(data => {
    ...
    return (
        <div className='px-5 md:px-52 min-h-screen bg-zinc-800 py-7 md:py-16'>
            ...
            {
                myReviews?.length === 0 ?
                    <div className='flex items-center justify-center pt-20 md:pt-44'>
