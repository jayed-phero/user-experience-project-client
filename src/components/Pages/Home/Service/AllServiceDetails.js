...
const AllServiceDetails = () => {
    const allSerb = useLoaderData()
    const { title, image, _id , desc } = allSerb;
    // console.log(allSerb)
    return (
        <div className='pt-32 bg-zinc-800'>
            <div>
                    ...
                    <div class="rounded-lg shadow-lg bg-white max-w-sm border border-gray-500 p-3 bg-transparent hover:border-green-500">
                        ...
                        <div class="px-3 pt-3">
                            ...
                            <div className='text-center'>
                                <Link to={`/details/${_id}`}><button type="button" class=" inline-block px-6 py-2.5 bg-green-500 text-white font-medium text-xs w-full leading-tight uppercase rounded shadow-md hover:bg-transparent border-2 border-green-500 hover:shadow-lg focus:bg-blue-700 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-blue-800 active:shadow-lg transition duration-150 ease-in-out">Details</button></Link>
                            </div>
                        </div>
                    </div>
                </div>
